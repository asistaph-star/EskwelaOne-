...
async function main() {
  ...
  // --- CREATE SECTION ---
  console.log('Creating Section Grade 7 Test Section...');
  let sectionData;
  try {
    ...
    console.log('Maybe exists, retrieving from db...');
    sectionData = await prisma.section.findFirst({ where: { name: 'Test Section' }});
  }
  ...
  try {
    teacherUser = await fetchApi('/users', {
      email: 'testteacher@digiskwela.com',
    ...
    console.log('Maybe exists, retrieving from db...');
    teacherUser = await prisma.user.findUnique({ where: { email: 'testteacher@digiskwela.com' }, include: { teacher: true }});
  ...
  try {
    studentUser = await fetchApi('/users', {
      email: 'teststudent@digiskwela.com',
      password: 'Password123!',
      firstName: 'Test',
      lastName: 'Student',
      roles: ['Student'],
    ...
    console.log('Maybe exists, retrieving from db...');
    studentUser = await prisma.user.findUnique({ where: { email: 'teststudent@digiskwela.com' }, include: { student: true }});
  }
  ...
  console.log('Teacher Email:', 'testteacher@digiskwela.com');
