...
export function NSidebar({ active, onNav, onLogout, collapsed=false }: {
  active: NScreen;
  onNav: (s: NScreen) => void;
  onLogout: () => void;
  collapsed?: boolean;
}) {
  const [showMenu, setShowMenu] = useState(false);
  const user = { name: "Maria Clara", sub: "School Nurse, Clinic Staff" };
  const initials = "MC";

  return (
    <div style={{ width: collapsed ? 70 : 240, background: C.m900, borderRight: `1px solid ${C.borderHeavy}`, display: "flex", flexDirection: "column", color: "#fff", transition: "width 0.2s", position: "relative", overflow: "hidden", flexShrink: 0 }}>
        ...
        {N_NAV_GROUPS.map(group => (
          <div key={group.category} style={{ marginBottom: 8 }}>
            ...
            {group.items.map(item => {
              ...
              return (
                <button key={item.id} onClick={() => onNav(item.id as NScreen)}
                  style={{
      ...
      <div style={{ padding: 16, borderTop: `1px solid ${C.borderHeavy}`, position: "relative", zIndex: 1 }}>
        ...
        {showMenu && (
          <div style={{ position: "absolute", bottom: "100%", left: 8, right: 8, marginBottom: 4, display: "flex", flexDirection: "column", gap: 4, padding: "6px", background: C.m800, borderRadius: 6, border: `1px solid ${C.borderHeavy}`, boxShadow: "0 -4px 16px rgba(0,0,0,0.3)", zIndex: 10 }}>
            <button onClick={() => { onNav('n-settings'); setShowMenu(false); }}
